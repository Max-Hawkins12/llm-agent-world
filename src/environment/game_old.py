...
from src.environment.utils import Position, is_within_bounds
...
class Game:
    ...
    def try_move_agent(self, dx: int, dy: int) -> None:
        target_position = self.agent.position.moved_by(Position(dx, dy))
        mob_at_target = self.get_mob_at(target_position)

        if not is_within_bounds(target_position, self.grid_width, self.grid_height):
        ...
        self.agent.move_to(target_position)
    ...
    def move_mobs(self) -> None:
        for mob in self.mobs:
            ...
            new_position = mob.next_move(self.grid_width, self.grid_height)

            if not self.is_cell_occupied(new_position):
                # A mob will not move onto an occupied cell
                mob.move_to(new_position)

    def is_cell_occupied(self, position: Position) -> bool:
        if self.agent.position == position:
            return True
        if self.weapon.alive and self.weapon.position == position:
            return True
        if self.goal.position == position:
            return True
        for mob in self.mobs:
            if mob.alive and mob.position == position:
                return True
        return False
    ...
    def get_mob_at(self, position: Position) -> Optional[Mob]:
        for mob in self.mobs:
            if mob.alive and mob.position == position:
                return mob
        return None
